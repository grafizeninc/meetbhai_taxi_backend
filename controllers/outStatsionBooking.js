const Booking = require("../models/outStationBooking");
...
exports.createBooking = async (req, res, next) => {
  try {
    ...
    await mainBooking.create({userId: savedBooking.user, outStationBookingId: savedBooking._id, bookingType: "outStation"});
...
exports.getBookingList = async (req, res, next) => {
  try {
    const bookings = await Booking.find({}).populate("user cityId categoryId");
    res.status(200).json({
      status: "success",
      data: { bookings },
    });
